...
from discogs_banner.canvas_tools import calculate_canvas, normalize_thumbs
...
class TestThumbRestrictions(unittest.TestCase):
    ...
    def setUp(self):
        self.thumbs = normalize_thumbs([random.randint(0, 150) for r in range(150)])
...
class TestCanvasSizes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.twobyone = calculate_canvas(
            [random.randint(0, 50) for r in range(50)], aspect="2x1"
        )
        self.fourbythree = calculate_canvas(
            [random.randint(0, 50) for r in range(50)], aspect="4x3"
        )
        self.sixteenbynine = calculate_canvas(
            [random.randint(0, 50) for r in range(50)], aspect="16x9"
        )
        self.invalidratio = calculate_canvas(
            [random.randint(0, 50) for r in range(50)], aspect="1x1"
        )
