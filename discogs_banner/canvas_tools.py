...
from discogs_banner.api_tools import fetch_collection, fetch_images, get_image_cache_filename
...
def create_image(config, image_name, thumbs, horizontal=1024, vertical=576):
    ...
    for i in range(0, horizontal, THUMB_WIDTH):
        for j in range(0, vertical, THUMB_WIDTH):
            ...
            try:
                # expects [id, title, url]
                thumb_item = thumbs.pop(random.randrange(len(thumbs)))
                image_file = get_image_cache_filename(thumb_item[0], thumb_item[2])
            except ValueError:
            ...
            # Use LANCZOS if available (Pillow 2.7+), fallback to ANTIALIAS (deprecated) if needed for very old versions.
            # We use getattr with None default to avoid AttributeError on modern Pillow where ANTIALIAS is removed.
            resample_method = getattr(Image, "LANCZOS", None)
            if resample_method is None:
                resample_method = Image.ANTIALIAS
            thumb.thumbnail((THUMB_WIDTH, THUMB_WIDTH), resample_method)
