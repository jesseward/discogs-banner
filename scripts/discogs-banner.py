...
from discogs_banner.canvas_tools import (ASPECT, calculate_canvas,
                                         create_image, normalize_thumbs)
...
def main(args, config):
    ...
    logger.info("Creating image={image}, at {h}x{v}".format(image=args.o, h=h, v=v))
    create_image(config, args.o, [x[2] for x in thumbs], horizontal=h, vertical=v)


if __name__ == "__main__":

    parser = argparse.ArgumentParser(
        description="Create image banner from Discogs album thumbs. Requires your Discogs collection to be populated (and public)"
    )
    parser.add_argument("user", type=str, help="Target Discogs account name")
    parser.add_argument(
        "-r",
        default="16x9",
        help="Aspect ratio for output image. Options are: {ratios}".format(
            ratios=", ".join([r for r in ASPECT])
        ),
    )
    parser.add_argument(
        "-o", default="discogs-banner.jpg", help="Output filename for rendered image."
    )
    parser.add_argument(
        "-c",
        default=os.path.expanduser("~/.config/discogs-banner/discogs-banner.conf"),
    ...
    if not os.path.exists(args.c):
        print(
            "Exiting, unable to locate config file {0}. use -c to specify config target".format(
                args.c
            )
        )
        sys.exit(1)
    ...
    config = configparser.ConfigParser(
        defaults={
            "cache_directory": os.path.expanduser(
                "~/.config/discogs-banner/image-cache"
            ),
            "log_file": os.path.expanduser(
                "~/.config/discogs-banner/discogs-banner.log"
            ),
    ...
    logging.basicConfig(
        filename=config.get("discogs-banner", "log_file"),
        format=FORMAT,
        level=logging.ERROR,
    )
    ...
    if not os.path.exists(config.get("discogs-banner", "cache_directory")):
        logger.warn(
            "Cache directory not found. Creating {cache_dir}".format(
